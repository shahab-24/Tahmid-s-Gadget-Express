...
const ProductDetails = () => {
  ...
  const handleAddToCart = (product) => {
    if (!cart.some((item) => item.product_id === product.product_id)) {
      ...
      toast.error(
        `Oops...! ${product.product_title} has already been added to your Cart...!`
      );
    }
  };
// add to wish function=============================================
  const handleAddToWishlist = (product) => {
    if (!wishlist.some((item) => item.product_id === product.product_id)) {
      ...
      toast.error(
        `Oops...! ${product.product_title} has already been added to your wishlist!`
      );
    }
  };
          ...
          <div className="card-body border-2 border-gray-700 rounded-xl space-y-6 w-[40%] h-full p-10 my-10">
            <div className="flex flex-col h-full">
              ...
              <div className="flex items-center gap-4 justify-centre ">
                <ReactStars
                  ...
                  activeColor="#ffd700"
                />{" "}
                <h2 className="bg-slate-50 opacity-55 rounded-full w-auto px-2 text-black">
                  {" "}
                  {rating}
                </h2>
              </div>
            ...
            <div className="card-actions justify-start flex items-center">
              ...
              <button
                ...
                className="btn bg-purple-600 rounded-full text-white font-semibold text-xl px-4"
              >
                Add to Cart
                <img
...
{/* add to wihslist button==========================================

*/}

              <button
                onClick={() => handleAddToWishlist(product)}
                className="btn bg-white opacity-55 rounded-full"
              >
                <img
                  src="https://img.icons8.com/?size=24&id=85038&format=png"
                  alt=""
                />
              </button>
            </div>
